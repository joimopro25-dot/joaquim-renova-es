      ...
      {projetos.length > 0 && (
        <section className="px-6 md:px-12 py-12 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {projetos.map((p) => {
              const geralCapa = p.fotos.find((f) => f.tipo === 'geral' && f.capa);
              ...
              const geral = geralCapa || p.fotos.find((f) => f.tipo === 'geral') || p.fotos[0];
              return (
                <button
                  key={p.id}
                ...
                >
                  {geralCapa ? (
                    <img src={geralCapa.url} className="w-full aspect-video object-cover" />
                  ) : antes && depois ? (
