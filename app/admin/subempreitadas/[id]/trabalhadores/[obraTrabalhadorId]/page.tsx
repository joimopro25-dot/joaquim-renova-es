...
export default function SubTrabalhadorDetalhe() {
  ...
  const carregar = useCallback(async () => {
    ...
    const [{ data: otData }, { data: entradasData }, { data: anexosData }, { data: ajustesData }] = await Promise.all([
      supabase.from('obra_trabalhadores').select('*, trabalhadores(nome, regime), subempreitadas(descricao)').eq('id', obraTrabalhadorId).single(),
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-ink-400">
            {ot.subempreitadas?.descricao} · {formatMoney(ot.valor_unitario)}{ot.tipo_valor !== 'fixo' ? `/${ot.tipo_valor === 'hora' ? 'h' : 'dia'}` : ''}
            {' · '}
