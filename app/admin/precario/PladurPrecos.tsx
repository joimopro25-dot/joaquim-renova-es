...
export default function PladurPrecos() {
  const [itens, setItens] = useState<Item[]>([]);
  const [loading, setLoading] = useState(true);

  async function carregar() {
    setLoading(true);
    const { data } = await supabase.from('pladur_precos').select('*').order('tipo').order('ordem');
    setItens(data || []);
    setLoading(false);
  }
  ...
  async function atualizarPreco(id: string, preco: number) {
    ...
    await supabase.from('pladur_precos').update({ preco }).eq('id', id);
  }
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-ink-400">Preços usados pelo Planeador de Pladur (materiais e mão de obra por m²). Ajusta livremente — atualiza logo o cálculo de novas simulações.</p>
              ...
              <tr key={i.id}>
                ...
                <td className="p-3 text-right">
                  <input
                    ...
                    onBlur={(e) => atualizarPreco(i.id, parseFloat(e.target.value) || 0)}
      ...
      <div className="card overflow-hidden">
        <div className="bg-sand-50 px-4 py-2 font-medium text-sm text-ink-700">Mão de Obra (€/m²)</div>
        <table className="w-full text-left text-sm">
          <thead className="text-ink-400 text-xs uppercase">
            <tr>
